...
export default {
  ...
  data: new SlashCommandBuilder()
    .setName('suggest-deny')
    .setDescription('Deny a suggestion by ID.')
  ...
  async execute(interaction) {
    ...
    if (updated.messageId && data.suggestions.channelId) {
      ...
      if (msg) {
        await msg.edit({
          embeds: [createSuccessEmbed(`Suggestion #${id} **DENIED**.\n${updated.text}`)]
        }).catch(() => {});
      }
    }
    ...
    return interaction.reply({
      embeds: [createSuccessEmbed(`Suggestion #${id} marked as denied.`)]
    });
  }
};
